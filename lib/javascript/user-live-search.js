cx.ready(function() {
    if ($J(".live-search-user-id").length > 0) {
        ...
        var replaceHTML = function(userInput) {
            ...
            let span = jQuery("<span class=\"live-search-user-title\" " + userTitleStyle + "></span>");
            span.append(document.createTextNode(userTitleVal));
            ...
            var replacedHTML = jQuery(replaceInput(userInput))
                .add(span)
                .add(anchor);

            if (typeof(userClear) != 'undefined') {
                replacedHTML.add(userClear);
            }
