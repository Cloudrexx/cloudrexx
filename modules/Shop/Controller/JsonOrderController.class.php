    ...
    implements \Cx\Core\Json\JsonAdapter
{
    ...
    public function setEmptyForPrice($params)
    {
        ...
        // TODO: this is wrong. Currency must be taken from order
    ...
    public function generateOrderItemShowView($params)
    {
        ...
        // discount that has been applied on shipment costs (if any)
        $shipmentDiscountAmount = 0;

        // note: there can only be one RelCustomerCoupon per Order
        foreach ($order->getRelCustomerCoupons() as $coupon) {
            if ($coupon->getAmount() > $netPrice) {
                $shipmentDiscountAmount = $coupon->getAmount() - $netPrice;
                $netPrice = 0;
            } else {
            ...
            $tableContent[] = array(
                ...
                'price' => '',
                'vat' => '',
                'sum' => '-' . ($coupon->getAmount() - $shipmentDiscountAmount),
                'orderAttributes' => ''
            );
        }
        ...
        if ($coupon && $shipmentDiscountAmount) {
            ...
            $customAttrs['shipmentAmount'] = $toolTipTrigger . $toolTipMessage . $customAttrs['shipmentAmount'];
        }

        // drop payment section in case the order did not use any payment
        if (!$order->getPayment()) {
            unset($customAttrs['paymentAmount']);
        }
